...
export default function ActivityPage(){return <PageShell active="activity"><main>
  ...
  <section className="activity-shortcuts" id="activity-list">{activities.slice(0,4).map(([icon,title,,tone])=><a className={tone} href={`#${title.toLowerCase().replaceAll(' ','-')}`} key={title}><i>{icon}</i><span>{title}</span><b>↗</b></a>)}</section>
